...
export const getPostsBySearch = async (req, res) => {
  ...
  try {
    const title = new RegExp(searchQuery, "i"); //test Test TEST will all be the same with i + using regexp coz its easier for mongodb to search

    const posts = await Post.find({
      $or: [{ title }, { tags: { $in: tags.split(",") } }],
    }); //either tag or title, tags IN array
    res.json({ data: posts });
  } catch (err) {
